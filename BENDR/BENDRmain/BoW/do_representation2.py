...
'''
@profile
def do_representation(dataSource, abspath, codebook_size, sub_length, inter_point, max_iterations, dataset='test', startingFileNo=0):
    ...
    if dataSource == 'csv':
        ...
        print('Loading data')
        for channelNo in range(0, noChannels):
            print(f'Channel {channelNo}:')
            
            

            for fileNo in range(startingFileNo, noFiles):
                print(f'  File: {fileNo}')
