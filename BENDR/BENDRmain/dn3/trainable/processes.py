...
class BaseProcess(object):
    ...
    def build_network(self, **kwargs):
        ...
        in the __init__.py method, they should be placed here.

        By default any arguments that propagate unused from __init__.py are included here.
        """
    ...
    def _make_dataloader(self, dataset, training=False, **loader_kwargs):
        ...
        if isinstance(dataset, DataLoader):
            return dataset
        if '_d' in loader_kwargs.keys():
            del loader_kwargs['_d']
    ...
    def fit(self, training_dataset, epochs=1, validation_dataset=None, step_callback=None,
            ...
            validation_interval=None, train_log_interval=None, save_downstream_checkpoint = False, load_downstream_checkpoint=False, **loader_kwargs):
        """
        ...
        if load_downstream_checkpoint:
            print('loading from checkpoint')
            downstreamCheckpoint = torch.load('checkpoints/downstream_checkpoint.pt')
            self.classifier.load_state_dict(downstreamCheckpoint['classifier'])
            resume_epoch = downstreamCheckpoint['epoch']
            self.optimizer.load_state_dict(downstreamCheckpoint['optimizer_state_dict'])
            self.scheduler = downstreamCheckpoint['scheduler']
            print(f'loaded checkpoint, epoch {resume_epoch}')
        ...
        for epoch in epoch_bar:
            ...
            if (epoch % 5 == 0) and (save_downstream_checkpoint): #save a checkpoint every 5 epochs
                torch.save({
                    'epoch': epoch,
                    'classifier': self.classifier.state_dict(),
                    'optimizer_state_dict': self.optimizer.state_dict(),
                    'scheduler': self.scheduler
                }, 'checkpoints/downstream_checkpoint.pt')
...
class StandardClassification(BaseProcess):
    ...
    def fit(self, training_dataset, epochs=1, validation_dataset=None, step_callback=None, epoch_callback=None,
            batch_size=8, warmup_frac=0.2, retain_best='loss', balance_method=None, load_downstream_checkpoint=False, save_downstream_checkpoint=False, **loader_kwargs):
        """
        sklearn/keras-like convenience method to simply proceed with training across multiple epochs of the provided
        dataset
        ...
        return super(StandardClassification, self).fit(training_dataset, epochs=epochs, step_callback=step_callback,
                                                       ...
                                                       balance_method=balance_method, load_downstream_checkpoint=load_downstream_checkpoint,
                                                       save_downstream_checkpoint=save_downstream_checkpoint,
                                                       **loader_kwargs)
