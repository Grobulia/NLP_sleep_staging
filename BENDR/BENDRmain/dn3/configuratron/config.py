...
class DatasetConfig:
    ...
    def _get_person_name(self, f: Path):
        if self.name == 'Physionet MMIDB':
            person = f.parent.name
        elif self.name == "Sleep_EDFx_Cassette":
            person = f.name[3:5]
        elif self.filename_format is None:
            person = f.parent.parent.parent.name
        else:
    ...
    def _get_session_name(self, f: Path):
        if self.name == 'Physionet MMIDB':
            ...
            sess_name = f.name[5]
        elif self.filename_format is not None and fnmatch(self.filename_format, "*{session*}*"):
            sess_name = search(self.filename_format, str(f))['session']
        else:
            sess_name = f.parent.parent.name
        return sess_name
