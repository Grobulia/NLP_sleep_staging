...
if __name__ == '__main__':
    ...
    #args = parser.parse_args(['linear', '--random-init', '--results-filename', 'results/linear_random_init.xlsx'])
    print(args)
        ...
        for fold, (training, validation, test) in enumerate(tqdm.tqdm(utils.get_lmoso_iterator(ds_name, ds))):
            if fold == 0:
                ...
                process.fit(training_dataset=training, validation_dataset=validation, warmup_frac=0.1,
                            retain_best=retain_best, pin_memory=False, load_downstream_checkpoint=False, save_downstream_checkpoint=True, **ds.train_params)
